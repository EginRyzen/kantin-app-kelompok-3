<nav class="mt-5 flex-1 px-2 space-y-1">
    ...
    <a href="{{ route('admin.cashiers.index') }}" 
       class="{{ request()->routeIs('admin.cashiers.*') ? 'bg-green-600 text-white' : 'text-gray-600 hover:bg-green-50 hover:text-green-700' }} 
              group flex items-center px-2 py-2 text-sm font-medium rounded-md"
       :class="isSidebarOpen ? '' : 'justify-center'">
        
        <svg class="mr-3 flex-shrink-0 h-6 w-6 {{ request()->routeIs('admin.cashiers.*') ? 'text-white' : 'text-gray-400 group-hover:text-gray-500' }}" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
