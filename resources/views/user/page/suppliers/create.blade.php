...
<section class="min-h-screen bg-gray-50 text-gray-900 py-8 px-4 sm:px-6">
    <div class="container mx-auto max-w-2xl">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <form action="{{ route('kasir.suppliers.store') }}" method="POST">
                @csrf
                ...
                <div class="mb-5">
                    ...
                    <input type="text" id="nama_supplier" name="nama_supplier" value="{{ old('nama_supplier') }}"
                        ...
                        placeholder="Contoh: PT. Sembako Jaya" required autofocus>
                    @error('nama_supplier')
                ...
                <div class="mb-6">
                    ...
                    <textarea id="alamat" name="alamat" rows="3"
                        class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-green-500 focus:border-green-500 block p-3.5 transition"
                        placeholder="Masukkan alamat supplier (opsional)...">{{ old('alamat') }}</textarea>
                    @error('alamat')
                        <p class="mt-2 text-xs text-red-600 font-medium">{{ $message }}</p>
                    @enderror
                </div>
